  ...
  const filterLinks = (links: IResources[]): IResources[] => {
    return links
      .map((link) => {
        ...
        const canAccessParent = ability.can("read", link.title);
        const canAccessSub = filteredSub && filteredSub.length > 0;
        return canAccessParent || canAccessSub ? { ...link, sub: filteredSub } : null;
      })
      ...
      <Layout fixed className={navOpened ? "h-svh" : ""}>
        ...
        >
          <div className={`flex items-center ${!isCollapsed ? "gap-2" : ""}`}>
            <Image src="/pern.webp" alt="Logo" width={60} height={60} className="rounded-full" />
            <div
          ...
          <Button variant="ghost" size="icon" className="md:hidden" aria-label="Toggle Navigation" aria-controls="sidebar-menu" aria-expanded={navOpened} onClick={() => setNavOpened((prev) => !prev)} >
            {navOpened ? <IconX /> : <IconMenu2 />}
          </Button>
        </Layout.Header>
        <Nav id="sidebar-menu" className={`z-40 h-full flex-1 overflow-auto ${ navOpened ? "max-h-screen" : "max-h-0 py-0 md:max-h-screen md:py-2" }`} closeNav={() => setNavOpened(false)} isCollapsed={isCollapsed} links={filteredLinks} />
          <Button onClick={() => setIsCollapsed((prev) => !prev)} size="icon" variant="outline" className="absolute -right-5 top-1/2 z-50 hidden rounded-full md:inline-flex" >
          <IconChevronsLeft
            stroke={1.5}
